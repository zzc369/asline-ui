  ...
  methods: {
    ...
    elementDown (e) {
      ...
      if (this.$el.contains(target)) {
        ...
        if (!this.enabled) {
          this.enabled = true
          this.$emit('activated', this.componentName)
    ...
    async handleUp () {
      ...
      if (this.resizing) {
        ...
        this.$emit('resizestop', this.left, this.top, this.width, this.height, this.componentName)
      }
      if (this.dragging) {
        this.dragging = false
        await this.conflictCheck()
        this.$emit('refLineParams', refLine)
        this.$emit('dragstop', this.left, this.top, this.componentName)
      }
      this.resetBoundsAndMouseState()
      removeEvent(document.documentElement, eventsFor.move, this.handleResize)
    },
